...
namespace XamarinChess.Controls
{
    public partial class ChessBoardView : Grid
    {
        ...
        public static readonly BindableProperty CommandParameterProperty = BindableProperty.Create<ChessBoardView, object>(p => p.CommandParameter, null);
        public static readonly BindableProperty CommandProperty = BindableProperty.Create<ChessBoardView, ICommand>(p => p.Command, null);
        public static readonly BindableProperty ItemsSourceProperty = BindableProperty.Create<ChessBoardView, IEnumerable<object>>(p => p.ItemsSource, null, BindingMode.OneWay, null, (bindable, oldValue, newValue) => { ((ChessBoardView)bindable).BuildTiles(newValue); });

        private int _maxColumns = 2;
        private float _tileHeight = 0;

        public Type ItemTemplate { get; set; } = typeof(ChessBoardTemplate);
